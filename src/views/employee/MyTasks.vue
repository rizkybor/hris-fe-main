...
const staffTaskCommentAssignees = computed(() => {
  ...
  const list = (detailStaffTask.value.assignees || []).map((a) => ({
    id: a.employee_id,
    name: a.name,
  }));
  ...
  if (creatorEmployeeId && !list.some((a) => a.id === creatorEmployeeId)) {
    list.push({
      ...
      name: detailStaffTask.value.creator?.name,
    });
  }
  return list;
});
...
const hasActiveFilters = computed(() =>
  Boolean(searchQuery.value || statusFilter.value || projectFilter.value),
);
const clearFilters = () => {
  searchQuery.value = "";
  statusFilter.value = "";
...
onMounted(async () => {
  ...
  // straight away.
  const targetId = route.query.staff_task
    ? Number(route.query.staff_task)
    : null;
  if (targetId) {
...
<template>
  <div class="px-4 py-4">
    <div class="bg-white sm:p-2 mb-3">
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="w-10 h-10 sm:w-11 sm:h-11 bg-orange-50 rounded-[10px] flex items-center justify-center shrink-0"
          >
            <ListChecks class="w-5 h-5 text-orange-600" />
          </div>
          <div>
            <h3 class="text-brand-dark text-base font-bold">
              Task Collection (Assigned/Project)
            </h3>
            <p class="text-brand-light text-xs">
              All tasks assigned to you are grouped by project or assignment
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
      ...
      <!-- ========================================================= -->
      <div
        v-if="staffTasksLoading || myStaffTasks.length > 0"
        class="bg-slate-50 border border-[#DCDEDD] rounded-[14px] p-4 sm:p-5"
      >
        <div class="flex items-center gap-2 mb-3.5">
          <div
            class="w-9 h-9 bg-emerald-50 rounded-[10px] flex items-center justify-center shrink-0"
          >
          ...
          <div class="min-w-0">
            ...
            <p class="text-brand-light text-xs">
              Daily tasks assigned to you -- verify your progress here
            </p>
          </div>
        </div>

        <!-- Loading -->
        <div v-if="staffTasksLoading" class="grid grid-cols-1 gap-3.5">
        ...
        <div v-else class="grid grid-cols-1 gap-3.5">
          <div
            ...
            :class="
              staffTaskStatusAccent[task.my_assignment?.status] ??
              'border-l-gray-300'
            "
          >
            <!-- Task Header -->
            <div class="flex items-start justify-between gap-2 mb-2.5">
              <h4 class="text-brand-dark text-sm font-semibold flex-1 min-w-0">
                {{ task.title }}
              </h4>
            ...
            <p
              ...
              class="text-brand-light text-xs mb-3 line-clamp-2"
            >
            ...
            <div class="flex items-center gap-1.5 mb-3 flex-wrap">
              ...
              <div
                class="flex items-center gap-1 px-2 py-0.5 rounded-md bg-gray-50 text-gray-600"
              >
      ...
      <!-- ========================================================= -->
      <div
        class="min-w-0 bg-slate-50 border border-[#DCDEDD] rounded-[14px] p-4 sm:p-5"
      >
        <div class="flex items-center gap-2 mb-3.5">
          <div
            class="w-9 h-9 bg-indigo-50 rounded-[10px] flex items-center justify-center shrink-0"
          >
            <ClipboardList class="w-4 h-4 text-indigo-600" />
          </div>

          <div class="min-w-0">
            <h4 class="text-brand-dark text-sm font-bold">Project Tasks</h4>
            <p class="text-brand-light text-xs">
              Tasks assigned to you from your projects
            </p>
          </div>
        </div>

        <!-- Project Tasks Header / Filter -->
        ...
        >
          <template v-if="hasActiveFilters">
            ...
            <p class="text-brand-light text-xs mb-4">
              Try adjusting your search, project, or status filter
            </p>
          ...
          <template v-else>
            ...
            <p class="text-brand-light text-xs">
              Tasks assigned to you will show up here, grouped by project
            </p>
          </template>
        </div>
          ...
          >
            ...
            <div class="grid grid-cols-1 gap-3.5">
              ...
              >
                ...
                <div class="flex items-start justify-between gap-2 mb-2.5">
                  <h4
                    class="text-brand-dark text-sm font-semibold flex-1 min-w-0"
                  >
                    {{ task.title }}
                  </h4>
    ...
    <!-- ========================================================= -->
    <div
      v-if="showStaffTaskDetail"
      class="fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center p-4"
      @click.self="closeStaffTaskDetail"
    >
      <div
        class="bg-white rounded-[14px] border border-[#DCDEDD] w-full max-w-2xl max-h-[90vh] overflow-y-auto"
      >
        <!-- Modal Header -->
        <div
          class="p-5 border-b border-[#DCDEDD] flex items-center justify-between"
        >
          <div class="min-w-0">
            <h3 class="text-brand-dark text-lg font-bold">
              {{ detailStaffTask?.title }}
            </h3>

            <p class="text-brand-light text-xs mt-0.5">
              Due {{ formatDate(detailStaffTask?.due_date) }} • by
              {{ detailStaffTask?.creator?.name }}
            </p>
          </div>

          <button
            @click="closeStaffTaskDetail"
        ...
        <div class="p-5 space-y-5">
          <!-- Description -->
          <div
            v-if="detailStaffTask?.description"
            class="rich-text-content text-sm text-brand-dark leading-relaxed"
            v-html="detailStaffTask.description"
          ></div>

          <!-- Status -->
          <div>
            <label class="text-sm font-semibold text-brand-dark mb-1.5 block">
              Your Status
            </label>
