...
const handleViewPdf = async (invoice) => {
  ...
  try {
    await store.viewPdf(invoice.id);
  } finally {
...
const handleCancel = async (invoice) => {
  if (
    !(await alertModal.confirm(`Cancel Invoice "${invoice.invoice_number}"?`, {
      type: "warning",
      confirmText: "Cancel",
    }))
  )
    ...
    <div class="bg-white p-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-brand-light text-sm">Manage client invoices</p>
        </div>
      </div>
      ...
      >
        ...
        <span class="text-brand-white text-sm font-semibold">Create Invoice</span>
      </router-link>
    </div>
    ...
    <div class="bg-white border border-[#DCDEDD] rounded-[14px] p-5">
      ...
      <div v-else class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead>
            <tr class="text-left text-brand-light border-b border-[#DCDEDD]">
              ...
              <th class="py-3 pr-4 font-semibold">Payment Receipt No.</th>
            ...
            >
              ...
              <td class="py-3 pr-4 font-mono text-xs">
                <template v-if="invoice.receipts?.length">
                  <div v-for="receipt in invoice.receipts" :key="receipt.id">{{ receipt.receipt_number }}</div>
                </template>
                <span v-else class="text-gray-400 font-sans">&mdash;</span>
              </td>
              ...
              <td class="py-3 pr-4">
                <div class="flex items-center justify-end gap-1">
                  ...
                  <router-link
                    ...
                    :to="{ name: 'admin.invoices.edit', params: { id: invoice.id } }"
        ...
        <div v-if="!loading && invoices.length === 0" class="text-center py-12 text-gray-500">
          <p class="text-lg font-semibold">No Invoices yet</p>
        </div>
      </div>
    ...
    <Transition name="fade">
      <div v-if="isDeleteModalOpen" class="fixed inset-0 z-[99] flex items-center justify-center p-4">
        ...
        <div class="bg-white rounded-[24px] p-6 w-full max-w-sm relative z-10 shadow-2xl">
          <h3 class="text-xl font-bold text-brand-dark mb-2">Delete Invoice?</h3>
          <p class="text-gray-500 text-sm mb-6">
            "{{ invoiceToDelete?.invoice_number }}" will be permanently deleted.
          </p>
