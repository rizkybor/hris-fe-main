...
import { useStaffTaskCommentStore } from "@/stores/staffTaskComment";
...
const props = defineProps({
  ...
  // [{ id, name }] -- the task's assigned staff (id = employee_profile id),
  // used both to gate the composer (canComment) and to power the @mention
  // dropdown. The creator is added separately by the caller if not already
  // an assignee, matching Meeting Note's "attendees + creator" pattern.
  assignees: {
    type: Array,
    default: () => [],
  },
...
const commentStore = useStaffTaskCommentStore();
...
const loadComments = () => {
  if (props.staffTaskId) commentStore.fetchComments(props.staffTaskId);
};
...
const closePickersOnOutsideClick = (event) => {
  if (
    ...
    !event.target.closest(".staff-task-comment-emoji-picker") &&
    !event.target.closest(".staff-task-comment-emoji-toggle")
  ) {
...
watch(() => props.staffTaskId, loadComments);
...
const selectMention = (assignee) => {
  ...
  if (!mentionedIds.value.includes(assignee.id)) {
    mentionedIds.value.push(assignee.id);
  }
...
const submitComment = async () => {
  ...
  try {
    await commentStore.createComment(props.staffTaskId, {
      body,
...
// ?comment=<id> on the task's detail route. Scroll that comment
...
watch(
  ...
  async ([val, targetId]) => {
    ...
    await nextTick();
    const el = document.getElementById(`staff-task-comment-${targetId}`);
    if (el) {
  ...
  <div class="bg-white border border-[#DCDEDD] rounded-[14px] p-5">
      ...
      <div v-for="thread in threads" :key="thread.root.id" class="space-y-3">
        ...
        <div class="flex gap-3 group">
          ...
          <div class="flex-1 min-w-0">
            <div
              :id="`staff-task-comment-${thread.root.id}`"
        ...
        <div v-if="thread.replies.length" class="pl-6 sm:pl-11 space-y-3 border-l-2 border-gray-100 ml-4">
          <div v-for="reply in thread.replies" :key="reply.id" class="flex gap-3 group">
            ...
            <div class="flex-1 min-w-0">
              <div
                :id="`staff-task-comment-${reply.id}`"
                class="bg-gray-50 rounded-[12px] px-3.5 py-2.5 transition-colors duration-300"
    ...
    >
      ...
      <p class="text-brand-light text-xs">
        Only the assigned staff, the task's creator, or managers can comment or mention others here.
      </p>
    </div>
      ...
      <div class="flex-1 relative">
        ...
        <div class="absolute bottom-2.5 right-2.5 flex items-center gap-1.5">
          <button
            ...
            class="staff-task-comment-emoji-toggle w-8 h-8 rounded-full flex items-center justify-center transition-colors"
        ...
        <Teleport to="body">
          ...
          >
            <button
              v-for="assignee in mentionResults"
              :key="assignee.id"
              type="button"
              @mousedown.prevent="selectMention(assignee)"
              class="w-full p-2.5 hover:bg-gray-50 transition-colors flex items-center gap-2.5 text-left"
            >
              <Avatar :alt="assignee.name" size="w-7 h-7" icon-size="w-3.5 h-3.5" />
              <div class="flex-1 min-w-0">
                <p class="text-brand-dark text-sm font-medium truncate">{{ assignee.name }}</p>
              </div>
        ...
        <Teleport to="body">
          <div
            ...
            class="staff-task-comment-emoji-picker bg-white border border-[#DCDEDD] rounded-[12px] shadow-2xl p-2 grid grid-cols-8 gap-1 w-64"
          >
            <button
